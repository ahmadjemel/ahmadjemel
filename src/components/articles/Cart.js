...
const Cart = () => {//composent cart
const cart = useSelector((state) => state.storecart);
const dispatch = useDispatch();//apres les nom du composent
const computeTotal = useCallback(() => {
dispatch(getTotals());
}, [cart, dispatch])//modifier lors de cart modifie ou dispatche se declanche
useEffect(() => {
computeTotal()
}, [computeTotal])

const handleAddToCart = useCallback((product) => {//tekhou produit 
...
<p>Your cart is currently empty</p>{/*pannier vide */}
...
{/*mate5dem ken maykounremplir le tableau */}
{cart.cartItems &&
...
<button className="clear-btn" onClick={() => handleClearCart()}>{/*in3aytou il metode w hya tkalem handel */}
Clear Cart
</button>
...
<button><Link to="/Checkout" style={{"color":"yellow"}}>Check
out</Link>
</button>
<button >
